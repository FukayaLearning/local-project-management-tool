...
- **作成・切替**: 複数のプロジェクトを作成し、切り替えて管理できます。プロジェクトごとにデータは独立して管理され、実体はGitブランチとして保存されます。

### 初期化フロー

- **初回起動**: データが存在しない場合、自動的にプロジェクト作成画面へ誘導されます。
- **Git初期化**: `data/` ディレクトリ配下に自動的にGitリポジトリが初期化されます。
