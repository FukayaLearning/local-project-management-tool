...
- **操作方法**
  ...
  - **CSVダウンロード**: 画面上の「CSVダウンロード」ボタンから、現在開いているプロジェクトの全タスクデータをCSV形式でエクスポートできます。Backlog等の外部ツールからインポートした際の未定義カラム（ツール側で表示されないフィールド）も維持された状態で出力されます。
...
- **操作方法**
  - **順序変更**: 左側のタスク行をドラッグ＆ドロップして並べ替えられます。並べ替えと同時に、他のタスクとの依存関係や担当者の工数を考慮して、以降のタスクのスケジュールが自動的に再計算（プレビュー）されます。
  - **スケジュールの適用**: 「Apply Schedule to CSV」ボタンを押下すると、現在計算されているスケジュール（プレビュー）が正式にタスクの開始日・期限日として適用され、CSVに保存されます。これにより、計算結果をBacklog等へ再インポート可能なデータとしてエクスポートできるようになります。
